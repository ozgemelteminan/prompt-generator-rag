...
export function AskDocumentsExperience() {
  ...
  return <section className="mx-auto max-w-5xl space-y-8 py-6 text-[#272A22]">
    <header className="space-y-3"><p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#6F7454]">PromptForge</p><h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">{text.title}</h1><p className="max-w-2xl text-base leading-7 text-[#747568]">{text.description}</p><button className="text-sm font-medium text-[#6F7454] underline underline-offset-4 focus:outline-none focus:ring-2 focus:ring-[#6F7454]" onClick={() => setLanguage((current) => current === "en" ? "tr" : "en")} type="button">{language === "en" ? "Türkçe" : "English"}</button></header>
    <form className="space-y-6 rounded-xl border border-[#D8D1C1] bg-[#FBF9F3] p-5 sm:p-7" onSubmit={handleAsk}>
      ...
      <div className="flex flex-wrap items-center gap-2 text-sm text-[#747568]"><span className="font-medium text-[#454A35]">{text.selected}:</span>{selectedDocuments.length ? selectedDocuments.map((document) => <span className="rounded-full bg-[#ECE6D8] px-3 py-1 text-[#454A35]" key={document.id}>{document.filename}</span>) : <span>{text.noneSelected}</span>}</div>
      <div className="space-y-2"><label className="text-sm font-semibold" htmlFor="document-question">{text.question}</label><textarea className="min-h-36 w-full rounded-xl border border-[#D8D1C1] bg-[#FBF9F3] p-4 leading-7 placeholder:text-[#747568] focus:border-[#6F7454] focus:outline-none focus:ring-2 focus:ring-[#6F7454]/30" disabled={isLoading} id="document-question" onChange={(event) => setQuestion(event.target.value)} placeholder={text.placeholder} value={question} /></div>
      <button className="rounded-lg bg-[#6F7454] px-5 py-3 font-semibold text-[#FBF9F3] transition hover:bg-[#5D6246] focus:outline-none focus:ring-2 focus:ring-[#6F7454] focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60" disabled={isLoading || readyDocuments.length === 0} type="submit">{isLoading ? text.asking : text.ask}</button>
    </form>
    {error && <p className="rounded-xl border border-[#B5574D]/40 bg-[#B5574D]/10 p-4 text-sm text-[#272A22]" role="alert">{localizedError(language, error.code, error.details)}</p>}
    {result?.state === "insufficient_evidence" && <p className="rounded-xl border border-[#B17A3A]/40 bg-[#FBF9F3] p-5 text-[#454A35]" role="status">{text.insufficient}</p>}
    {result?.state === "answer" && result.answer && <section className="space-y-7"><article className="rounded-xl border border-[#D8D1C1] bg-[#FBF9F3] p-5 sm:p-7"><h2 className="text-xl font-semibold">{text.answer}</h2><p className="mt-4 whitespace-pre-wrap leading-7">{renderAnswer(result.answer, focusCitation)}</p></article><section className="space-y-3"><h2 className="text-xl font-semibold">{text.sources}</h2>{result.sources.map((source) => <SourceCard highlighted={highlightedCitation === source.citationId} key={source.citationId} source={source} text={text} />)}</section></section>}
  </section>;
}
